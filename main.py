...
if __name__ == '__main__':
    ...
    top_features_indices = np.argsort(importances)

    for model, model_name in models.items():

        # Loop over the number of "most important feature" to include
        for N in range(3, importances.shape[0]):
            top_train_inputs = train_inputs[:, top_features_indices[-N:]]
            top_dev_inputs = dev_inputs[:, top_features_indices[-N:]]
